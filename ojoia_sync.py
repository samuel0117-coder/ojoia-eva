...
class OjoiaSync:
    ...
    def push_billing(self, billing: dict) -> None:
        """Sincroniza datos de billing (stats, clients, config, reqlog) a Firestore.

        Estructura en Firestore:
          /billing/global/stats/{window}    -> stats agregados (24h, 7d)
          /billing/global/clients_meta      -> lista de clientes con uso
          /billing/global/config            -> precios y planes
          /billing/global/reqlog/{id}       -> requests recientes (max 200)

        Throttle: este nodo SOLO sincroniza billing si lo tiene local
        (billing.py instalado). El otro nodo (ojoia) tiene su propia instancia.
        """
        if not self.enabled:
            return
        try:
    ...
    def billing_provider(self) -> dict:
        """Retorna un provider de billing para run_loop.

        Si billing.py NO está instalado en este nodo, retorna {}.
        Cineia típicamente no tiene billing local (lo tiene ojoia).
        """
        try:
            import sys as _sys
            for p in ("/opt/ojoia/code", "/home/sam/ai_system", "/home/sam/ojoia-eva"):
                if p not in _sys.path:
                    _sys.path.insert(0, p)
            from billing import BillingStore
            from billing_log import get_stats, get_requests
            ...
            # Normal: cineia no tiene billing local
            return {}
    ...
    async def run_loop(self, status_provider, executor, shutdown=None,
                       billing_provider=None, billing_interval: float = 180.0,
                       status_interval: float = 15.0):
        """Bucle principal: push status + poll control cada `status_interval` s.

        Throttle (commit 730c68d — evita 429 Quota exceeded en Firestore free tier):
          - status: cada 15s (default)
          - billing: cada 180s (3 min) si billing_provider está definido

        Si billing_provider está definido, sincroniza billing cada
        `billing_interval` segundos.
        """
        if not self.enabled:
            print("[ojoia_sync] Bucle no iniciado (deshabilitado)")
            return
        # Usar el intervalo pasado (por si el __init__ lo dejó distinto)
