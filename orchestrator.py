...
def _detect_attention_hits(vision: dict, attention_phrases: list, owner_notes: list,
    ...
    if is_after_hours and mode == "normal":
        persons_count = 0
        if isinstance(vision, dict):
            # F0.2: el JSON de Qwen usa "persons" (es/en mixto según _convert_qwen_vision_response).
            # Antes solo se leía "personas" → siempre 0 → la alerta nunca disparaba.
            persons_raw = vision.get("persons", vision.get("personas", []))
            if isinstance(persons_raw, list):
                persons_count = len(persons_raw)
            elif isinstance(persons_raw, (int, float)):
                persons_count = int(persons_raw)
