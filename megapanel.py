...
# Antes había una lista hardcoded con servicios que pueden no existir. Ahora
# escaneamos los .service files en /etc/systemd/system/ y los contenedores
# Docker reales para mostrar solo los que están instalados en este nodo.
# Los servicios del nodo remoto se descubren via el agent (CINEIA_AGENT_URL).
...
_KNOWN_SYSTEMD_SERVICES = {
    ...
    "admin_panel.service": {"name": "ChatRD Admin (legacy)", "port": 8030, "gpu": -1, "kind": "api"},
...
_KNOWN_DOCKER_CONTAINERS = {
    ...
    "cineia-h3-worker":  {"name": "MiniMax H3 Worker", "port": 8000, "gpu": -1, "kind": "video", "id": "cineia-h3-worker"},
    "cineia-flux":       {"name": "Flux GGUF", "port": 8020, "gpu": 2, "kind": "image", "id": "cineia-flux"},
    "cineia-f5tts":      {"name": "F5-TTS (CPU)", "port": 8017, "gpu": -1, "kind": "audio", "id": "cineia-f5tts"},
    "cineia-musicgen":   {"name": "MusicGen (CPU)", "port": 8023, "gpu": -1, "kind": "audio", "id": "cineia-musicgen"},
    "cineia-realesrgan": {"name": "RealESRGAN (CPU)", "port": 8021, "gpu": -1, "kind": "upscale", "id": "cineia-realesrgan"},
    "cineia-stable-audio": {"name": "Stable Audio", "port": 8025, "gpu": -1, "kind": "audio", "id": "cineia-stable-audio"},
    "cineia-sadtalker":  {"name": "SadTalker", "port": 8026, "gpu": -1, "kind": "lipsync", "id": "cineia-sadtalker"},
    "cineia-redis":      {"name": "Redis (cola)", "port": 6379, "gpu": -1, "kind": "infra", "id": "cineia-redis"},
}
...
def _discover_systemd_services() -> list:
    ...
    Filtra para mostrar solo servicios relevantes de OjoIA (los que están
    en _KNOWN_SYSTEMD_SERVICES o contienen palabras clave del stack).
    """
    # Si el id está en _KNOWN_SYSTEMD_SERVICES, lo mostramos.
    # Si NO está, lo mostramos solo si contiene palabras clave de OjoIA
    # para evitar ruido (dbus, chronyd, vmtoolsd, etc).
...
def _discover_docker_containers() -> list:
    ...
    for line in out.splitlines():
        ...
        services.append({
            "id": meta.get("id", container),
            "name": meta.get("name", container),
            "port": meta.get("port", 0),
            "gpu": meta.get("gpu", -1),
            "kind": meta.get("kind", "docker"),
            "level": "docker",
            "node": NODE_ID,
            "container": container,
            "managed": "docker",
...
# Se calcula al arranque (no es una constante hardcoded).
