...
def _load():
    ...
    from transformers import (
        ...
        RobertaTokenizerFast, T5TokenizerFast, ClapFeatureExtractor,
        SpeechT5HifiGan,
    )
    ...
    tokenizer_2 = T5TokenizerFast.from_pretrained(os.path.join(m, "tokenizer_2"), extra_special_tokens={})
...
async def unload():
    ...
    try:
        ...
        pass
    return {"status": "unloaded", "model": "audioldm2", "loaded": False}


@app.post("/api/tts/unload")
async def unload_alias():
    return await unload()
...
async def load():
    try:
        get_pipe()
        return {"status": "loaded", "model": "audioldm2", "loaded": True}
    except Exception as e:
