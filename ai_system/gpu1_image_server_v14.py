...
APP = FastAPI(title="GPU1 Image Server", version="1.4")
...
def load_vae_fp16_fix(pipe_obj):
    ...
    if os.path.exists(vae_path):
        ...
        pipe_obj.vae = vae
        print(f"[gpu1] VAE sdxl_vae loaded via AutoencoderKL v1.4", flush=True)
    else:
...
def startup():
    ...
    threading.Thread(target=lambda: load_sdxl_model("turbo"), daemon=True).start()
