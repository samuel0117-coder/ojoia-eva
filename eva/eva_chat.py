...
def _create_session(session_id: str, user_id: str,
    ...
    if cam_id and cam_id != "unknown":
        ...
        if cam_cfg.get("zone"):
            ...
            if existing_rules:
                s["confirmed_rules"] = existing_rules
                s["qwen_rules"] = existing_rules
                s["phase"] = EvaPhase.DONE.value
    # Si el usuario ya tiene cámaras configuradas pero viene sin cam_id, saltar a DONE
    if s["phase"] == EvaPhase.GREET.value:
        try:
            user_file = storage_root / "users" / user_id / "user.json"
            if user_file.exists():
                with open(user_file) as _f:
                    _ud = json.load(_f)
                _cams = _ud.get("cameras", [])
                if _cams:
                    s["phase"] = EvaPhase.DONE.value
                    s["camera_id"] = _cams[0].get("camera_id", "")
                    s["zone"] = _cams[0].get("zone", "")
                    s["camera_connected"] = True
                    s["position_confirmed"] = True
            if s["phase"] == EvaPhase.DONE.value:
                s["_greeted"] = True
        except:
            pass
