...
const EvaChat = {
    ...
    // ── HTTP con token (mismo apiFetch que app-v12.js) ──────────
    _apiFetch(url, opts = {}) {
        if (typeof window.apiFetch === 'function') return window.apiFetch(url, opts);
        const headers = { 'Content-Type': 'application/json', ...(opts.headers || {}) };
        if (App && App.accessToken) headers['Authorization'] = 'Bearer ' + App.accessToken;
        return fetch(url, { mode: 'cors', headers, ...opts });
    },

    // ── INIT ────────────────────────────────────────────────────
    async init(userId, userName) {
        if (this._initialized && this.userId === userId) {
            // Re-entrant ( cambio de tab ): solo re-renderizar, no resetear
            this.render();
            return;
        }
    ...
    async sendMessage() {
        ...
        try {
            const chatSessionId = this._isOsIntentText(intentText) && !this._isInstallCameraIntent(intentText)
                ? `os_${this.userId}`
                : this.sessionId;
            const r = await this._apiFetch(`${this.API}/config/chat`, {
                method: 'POST',
    ...
    async showAlertEvent(eventId) {
        if (!eventId) return;
        try {
            const r = await this._apiFetch(`${this.API}/api/events/${eventId}?user_id=${this.userId}`);
            if (!r.ok) return;
...
try {
    ...
    async sendFeedback(eventId, isReal, btn) {
        try {
            await this._apiFetch(`${this.API}/api/chat/eva/feedback`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ user_id: this.userId, event_id: eventId, is_real: isReal }) });
    ...
    async _buildDailyBrief() {
        ...
        const eventsR = await this._apiFetch(`${this.API}/api/user/events?user_id=${this.userId}&filter=today&limit=20`);
    ...
    scrollToBottom(force = false) {
        ...
        // Forzar inmediatamente y tras layout (contenedor flex puede tener height 0)
        doScroll();
        requestAnimationFrame(() => {
            ...
            if (force) setTimeout(doScroll, 1000);
        });
    ...
    async _readStatus() {
        try {
            const [camsR, evtsR] = await Promise.all([
                this._apiFetch(`${this.API}/api/cameras?user_id=${this.userId}`),
                this._apiFetch(`${this.API}/api/user/events?user_id=${this.userId}&filter=today&limit=1`)
            ]);
    ...
    async loadBusinessContext() {
        try {
            const r = await this._apiFetch(`${this.API}/api/user/profile?user_id=${this.userId}`);
            if (r.ok) {
