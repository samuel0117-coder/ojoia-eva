...
const App = {
    ...
    async _verifyFB(user, extra = {}) {
        try {
            ...
            const r = await apiFetch(apiUrl + '/auth/firebase/verify', { method: 'POST', body: JSON.stringify(body) });
...
async _fetchFrameForCam(camId) {
    ...
    async _saveCooldown(camId) {
        ...
        try {
            const r = await apiFetch(`${this.API}/admin/eva-config`, {
                method: 'POST',
                body: JSON.stringify({ violation_cooldown_min: val })
            });
            const d = await r.json();
            if (d.success) {
