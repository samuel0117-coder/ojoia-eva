...
const App = {
    ...
    init() {
        ...
        // Timeout fallback si Firestore no responde en 3s
        setTimeout(() => {
    ...
    async _openCameraConfig(camId) {
        ...
        try {
            ...
            const localIP = cam.local_ip || cam.ip || null;
            const fs = cam.active ? 'Online' : 'Offline';
            const fsColor = cam.active ? 'var(--success)' : 'var(--danger)';
            let localOK = false;
            if (localIP) {
                try {
                    const lr = await fetch(`http://${localIP}:81/status`, {mode:'cors', signal: AbortSignal.timeout(3000)});
                    if (lr.ok) localOK = true;
                } catch(e) {}
            }
            c.innerHTML = 
                '<div style="display:flex;flex-direction:column;height:100%;overflow-y:auto;padding:16px;">' +
                    '<div style="display:flex;align-items:center;gap:12px;margin-bottom:20px;">' +
                        ...
                        '<div><div style="font-weight:600;font-size:1.1rem;">Configuración de cámara</div>' +
                        '<div style="font-size:0.8rem;color:var(--text-secondary);">' + cam.name + (localOK ? ' · 🟢 Local ✓' : '') + '</div></div>' +
                    '</div>' +
                        '<div><div style="font-weight:600;font-size:1.1rem;">' + cam.name + '</div>' +
                        '<div style="color:' + fsColor + ';font-size:0.85rem;">' + fs + (localOK ? ' · Local ✓' : '') + '</div></div>' +
                    '</div>' +
                    '<div style="font-size:0.8rem;color:var(--text-secondary);margin-bottom:16px;padding:12px;background:var(--bg-secondary);border-radius:8px;">' +
                        '📊 eventos: ' + (cam.metrics?.total_events || 0) + ' · alertas: ' + (cam.metrics?.total_alerts || 0) + '<br>' +
                        '📏 reglas: ' + (cam.rules?.length || 0) + ' activas<br>' +
                        '⏰ horario: ' + (cam.schedule?.open || '07:00') + ' - ' + (cam.schedule?.close || '19:00') +
                    '</div>' +
                    '<div style="font-weight:600;margin:20px 0 12px;">Rotación de imagen</div>' +
                    '<button class="btn" onclick="App._sendCamCmd(\''+camId+'\',\'rotation\',\'next\')" style="width:100%;padding:14px;font-size:1rem;">↻ Rotar 90°</button>' +
                    '<p class="meta" style="margin:8px 0 20px;text-align:center;">Cicla: 0° → 90° → 180° → 270° → 0°</p>' +
                    '<div style="font-weight:600;margin-bottom:10px;">Calidad</div>' +
                    ...
                    '</div>' +
                    '<div style="font-weight:600;margin-bottom:10px;">💡 Iluminación</div>' +
                    ...
                    '</div>' +
                    '<p class="meta" style="margin-top:4px;margin-bottom:20px;">Auto: ajusta según luz ambiente</p>' +
    ...
    async _sendCamCmd(camId, cmd, val) {
        try {
            const r = await apiFetch(`${this.API}/api/cameras?user_id=${this.userId}`);
            const d = await r.json();
            const cam = (d.cameras || []).find(c => c.camera_id === camId);
            const localIP = cam?.local_ip || cam?.ip;
            if (!localIP) { this._toast('','Conéctate a la misma red WiFi que la cámara','warning'); return; }
            
            const url = `http://${localIP}:81/config`;
            let body = {};
            
            if (cmd === 'rotation') {
                // Rotación cíclica: obtiene la rotación actual y avanza 90°
                let current = cam?.rotation || 0;
                if (val === 'next') {
                    current = (current + 1) % 4;  // Cicla 0→1→2→3→0
                } else {
                    current = val;
                }
                body = {rotation: current};
                this._toast('','Rotación: ' + (current * 90) + '°','success');
            } else if (cmd === 'quality') {
                body = {quality: val};
                this._toast('','Calidad actualizada','success');
            } else if (cmd === 'led') {
                body = {led_auto: false, led_on: val ? 1 : 0};
                this._toast('','LED ' + (val ? 'encendido' : 'apagado'),'success');
            } else if (cmd === 'led_auto') {
                body = {led_auto: true};
                this._toast('','LED automático activado','success');
            } else if (cmd === 'snapshot') {
                window.open(`http://${localIP}:81/snapshot`, '_blank');
                return;
            }
            
            await fetch(url, {method:'POST', body:JSON.stringify(body), mode:'cors', signal: AbortSignal.timeout(5000)});
        } catch(e) {
            this._toast('','Error: la cámara debe estar en la misma WiFi','danger');
        }
    },
    ...
    async _loadEvents(filter) {
        ...
        try {
            ...
            el.innerHTML = evts.map(evt => {
                ...
                const camName = evt.camera_name || 'Sin nombre';
                ...
                return `<div class="event-row ${violation ? 'event-alert' : ''}" onclick="App._openEvent('${evt.event_id}')">
                    <div class="event-thumb" id="evthumb-${evt.event_id}" style="background:${iconBg};">
                        <span style="font-size:1.3rem;">${icon}</span>
                    </div>
                    <div class="event-info">
                        <div class="event-title">${camName}${camZone ? ' · ' + camZone : ''}</div>
                        <div class="meta">${ts} · YOLO: ${yoloCount} obj.</div>
                        <div class="meta event-desc" style="margin-top:2px;font-size:0.78rem;color:var(--text-secondary)">${desc}</div>
                    </div>
...
_showSubscription() {
    ...
    _showEvaConfig(config) {
        ...
        // Store rules for later use by onclick handlers
