    ...
    async _loadEvents(filter) {
        ...
        try {
            ...
            el.innerHTML = evts.map(evt => {
                ...
                // Limpiar texto del template de Qwen (si quedó en inglés)
                let desc = rawDesc
                    .replace(/- If ALL checks NO.*$/m, '')
                    .replace(/^No violation detected.*$/i, 'Sin actividad sospechosa')
                    .replace(/^Error analizando.*$/i, 'Sin actividad sospechosa')
                    .trim();
...
_showSubscription() {
    ...
    async _installApp() {
        if (!this._deferredPrompt) {
            ...
            if (isIOS) {
                alert('Para instalar en iOS: toca el botón Compartir (⬆️) y luego "Agregar a inicio"');
            } else {
