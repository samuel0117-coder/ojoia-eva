...
#define DEFAULT_SERVER_URL  "https://api.ojoia.com.do"
#define DEFAULT_INTERVAL_MS  1000
#define DEFAULT_QUALITY      12
#define DEFAULT_FRAMESIZE    FRAMESIZE_VGA
#define DEFAULT_LED_AUTO     true
#define DEFAULT_LED_BRIGHT   200
#define DEFAULT_H_MIRROR     false
#define DEFAULT_V_FLIP       false

#define LIGHT_LOW_THRESH     80
#define LIGHT_HIGH_THRESH   120
...
#define NTP_TZ       "America/Santo_Domingo"
#define NTP_TZ_OFF   (-4 * 3600)

Preferences prefs;
WebServer   portalServer(80);
WebServer   configServer(81);
DNSServer   dns;
...
struct CamConfig {
  ...
  uint8_t  framesize    = DEFAULT_FRAMESIZE;
  bool     led_auto     = DEFAULT_LED_AUTO;
  uint8_t  led_bright   = DEFAULT_LED_BRIGHT;
  bool     h_mirror     = DEFAULT_H_MIRROR;
  bool     v_flip       = DEFAULT_V_FLIP;
  bool     stream_always = true;
} cfg;
...
const char PORTAL_HTML[] PROGMEM = R"RAW(<!DOCTYPE html><html lang="es"><head>
<meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1">
<title>OjoIA Setup</title>
<style>
:root{--bg:#000;--card:#111;--b:#1c1c1e;--txt:#fff;--mu:#636366;--bl:#0a84ff}
*{box-sizing:border-box;margin:0;padding:0}
body{background:var(--bg);color:var(--txt);font-family:-apple-system,sans-serif;min-height:100vh;display:flex;flex-direction:column;align-items:center;padding:40px 16px}
.logo{font-size:48px;margin-bottom:10px}h1{font-size:22px;font-weight:600}
.tag{font-size:11px;color:var(--mu);margin:4px 0 30px;letter-spacing:1px;text-transform:uppercase}
.card{width:100%;max-width:400px;background:var(--card);border:1px solid var(--b);border-radius:16px;padding:20px}
.nets{max-height:200px;overflow-y:auto;border-radius:10px;background:var(--bg);border:1px solid var(--b);margin-bottom:12px}
.net{display:flex;justify-content:space-between;align-items:center;padding:12px 14px;cursor:pointer;border-bottom:1px solid var(--b);transition:background .15s}
.net:hover{background:#0e0e0e}.net.sel{background:#011126;border-left:3px solid var(--bl)}
.net .nm{font-size:14px}.net .sg{font-size:11px;color:var(--mu)}
.si{font-size:12px;color:var(--bl);min-height:16px;margin-bottom:12px}
lbl{display:block;font-size:10px;color:var(--mu);letter-spacing:.9px;text-transform:uppercase;margin:10px 0 4px}
input{width:100%;padding:12px;border-radius:10px;border:1.5px solid var(--b);background:var(--bg);color:var(--txt);font-size:15px;outline:none}
input:focus{border-color:var(--bl)}
.btn{width:100%;padding:13px;margin-top:18px;border:none;border-radius:12px;font-size:15px;font-weight:600;background:var(--bl);color:#fff;cursor:pointer}
.btn:disabled{opacity:.3;cursor:default}
.ok{text-align:center;padding:20px 0}.ok .ico{font-size:48px;margin-bottom:14px}.ok h2{font-size:20px;margin-bottom:8px}.ok p{font-size:13px;color:var(--mu);line-height:1.6}
</style></head><body>
<div class="logo">👁️</div><h1>OjoIA</h1>
<div class="tag">Configuración de red</div>
<div class="card" id="setupCard">
<div style="font-size:10px;color:var(--mu);letter-spacing:.9px;text-transform:uppercase;margin-bottom:10px">Redes WiFi disponibles</div>
<div class="nets" id="nets"><div style="padding:20px;text-align:center;color:var(--mu);font-size:13px">Buscando...</div></div>
<div class="si" id="si"></div>
<lbl>Contraseña WiFi</lbl>
<input type="password" id="pw" placeholder="Contraseña" oninput="chk()" autocomplete="new-password">
<lbl style="margin-top:16px">URL del servidor</lbl>
<input type="text" id="srv" value="https://api.ojoia.com.do">
<button class="btn" id="b1" disabled onclick="save()">Conectar →</button>
</div>
<div id="okCard" class="card ok" style="display:none">
<div class="ico">✅</div><h2>¡Conectado!</h2><p>El dispositivo se reiniciará en 3 segundos<br>y comenzará a transmitir.</p>
</div>
<script>
var ss=null;
function sig(r){return r>=-55?'████':r>=-65?'███▁':r>=-75?'██▁▁':'█▁▁▁';}
function scan(){
  fetch('/scan').then(r=>r.json()).then(d=>{
    var el=document.getElementById('nets');el.innerHTML='';
    var n=d.networks||[];
    if(!n.length){el.innerHTML='<div style="padding:20px;text-align:center;color:var(--mu)">Sin redes</div>';return;}
    n.forEach((x,i)=>{
      var dv=document.createElement('div');dv.className='net';dv.dataset.i=i;
      dv.innerHTML='<span class="nm">'+(x.sec?'🔒 ':'')+x.ssid+'</span><span class="sg">'+sig(x.rssi)+'</span>';
      dv.onclick=function(){
        ss=n[this.dataset.i].ssid;
        document.querySelectorAll('.net').forEach(e=>e.classList.remove('sel'));
        this.classList.add('sel');
        document.getElementById('si').textContent='→ '+ss;
        chk();
      };
      el.appendChild(dv);
    });
  }).catch(()=>{document.getElementById('nets').innerHTML='<div style="padding:20px;text-align:center;color:#ff453a">Error buscando redes</div>';});
}
function chk(){
